...
**设计说明/归档：** 不涉及新的系统拓扑，沿用现有 MindSpore 实训环境入口、Pod 绑定信息和账号中心，仅移除鉴权缓存复用逻辑并改为实时鉴权。
...
**设计说明/归档：** 访问请求携带的 `UT/YG` 和环境标识只用于实时鉴权，不缓存可跨请求复用的授权结果。

```mermaid
flowchart LR
    Req[请求: UT/YG + env URL] --> Extract[提取 envId]
    Extract --> Query[查询账号中心/权限源]
    Query --> UserName[解析 UT/YG 对应用户名]
    UserName --> Verify{用户名是否与 Pod 绑定/owner 一致}
...
| Pod 绑定信息 | 保存环境所属用户名/owner 名称 | 环境标识 | 绑定用户名/owner |
| 账号中心/权限源 | 提供当前用户对目标环境的真实授权结果 | 用户标识、环境标识 | 授权通过/拒绝 |

**接口约定：**
- 鉴权中间件必须每次完成当前请求主体识别并实时查询账号中心，解析出 `UT/YG` 对应用户名。
- 鉴权结果必须和 Pod 绑定用户名/owner 名称进行比对。
...
**设计说明/归档：** 本需求的主要风险来自“鉴权结果复用”和“跨用户 URL 访问”，核心信任边界是用户请求、实训环境入口、账号中心和 Pod 绑定信息。
...
|-----------|-------------------------|---------------|----------------------------|
| **信息泄露** | 通过共享 URL 进入他人实训环境，读取或修改环境中的数据 | 高 | 每次访问都实时解析 `UT/YG` 对应用户名，并与 Pod 绑定/owner 比对 |
...
本需求采用“UT/YG 对应用户名 + 当前环境 + Pod 绑定用户名/owner”三方实时校验，移除缓存结果成为跨用户越权入口的可能性。

* **身份认证与授权**：从现有请求中提取 `UT/YG`，进入实训环境时必须实时解析出用户名，并与 Pod 绑定用户名/owner 进行比对。
* **缓存移除**：删除鉴权缓存数据结构、命中分支和写入逻辑，不再保存可跨请求复用的授权结果。
* **失败处理**：鉴权失败直接返回 401/403，不继续执行环境进入逻辑；不返回内部实现细节。
* **审计日志**：记录 `UT/YG`、env_id、解析出的用户名、Pod 绑定用户名/owner、授权结果、原因码；不记录完整 token、cookie 或环境敏感内容。
