# #151 MindSpore实训沙箱鉴权缓存移除与实时校验 架构设计说明书
...
**设计说明/归档：** 不涉及新的系统拓扑，沿用现有 MindSpore 实训环境入口、Pod-ID 绑定信息和账号中心，仅移除鉴权缓存复用逻辑并改为实时鉴权。
...
| 账号中心/权限源 | 提供 `UT/YG` 对应的 username | `UT/YG` | username |

**接口约定：**
- 鉴权中间件必须每次完成当前请求主体识别并实时查询账号中心，解析出 `UT/YG` 对应 username。
...
**设计说明/归档：** 本需求的主要风险来自“鉴权结果复用”和“跨用户 URL 访问”，核心信任边界是用户请求、实训环境入口、账号中心和 Pod-ID 绑定信息。
...
* **失败处理**：鉴权失败直接返回 403，不继续执行环境进入逻辑；不返回内部实现细节。
* **审计日志**：记录 Pod-ID、解析出的 username、ownername、授权结果、原因码；不记录完整 token、cookie 或环境敏感内容。
...
| **TASK3** | 补充审计日志和错误码，编写越权访问场景测试验证跨用户 URL 复用被拦截 | chenqi |
...
| TASK3 | 补充审计与错误码，完成越权访问测试验证 | 可追踪的失败路径与测试记录 | chenqi |
