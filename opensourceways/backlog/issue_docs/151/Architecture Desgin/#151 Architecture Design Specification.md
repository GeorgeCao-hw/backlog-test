...
**设计说明/归档：** 不涉及新的系统拓扑，沿用现有 MindSpore 实训环境入口、Pod-ID 绑定信息和 authen center，仅移除鉴权缓存复用逻辑并改为实时鉴权。

```mermaid
flowchart LR
    U[用户浏览器] --> G[实训环境入口]
    G --> A[鉴权中间件]
    A --> S[实时查询 authen center]
    S --> N[提取 UT/YG 对应用户名]
    N --> R{用户名是否与 Pod-ID 绑定 ownername 一致?}
    R -- 是 --> P[放行进入环境]
    R -- 否 --> F[拒绝访问]
```
...
**设计说明/归档：** 访问请求携带的 `UT/YG` 和 `Pod-ID` 只用于实时鉴权，不缓存可跨请求复用的授权结果。

```mermaid
flowchart LR
    Req[请求: UT/YG + Pod-ID] --> Extract[提取 Pod-ID]
    Extract --> Query[查询 authen center/权限源]
    Query --> UserName[解析 UT/YG 对应 username]
    UserName --> Verify{username 是否与 Pod-ID 绑定 ownername 一致}
...
| 组件 | 职责 | 输入 | 输出 |
|------|------|------|------|
| 实训环境入口鉴权中间件 | 读取当前登录用户、环境 ID，执行访问校验 | Cookie/Session、请求 URL、Pod-ID | 放行/拒绝结果 |
| Pod-ID 绑定信息 | 保存环境所属 ownername | Pod-ID | ownername |
| authen center/权限源 | 提供 `UT/YG` 对应的 username | `UT/YG` | username |

**接口约定：**
- 鉴权中间件必须每次完成当前请求主体识别并实时查询 authen center，解析出 `UT/YG` 对应 username。
- xihe-server 负责将 username 和 Pod-ID 绑定的 ownername 进行比对。
...
| **TASK2** | 改造环境访问链路为实时鉴权，每次访问回源 authen center | chenqi |
...
**设计说明/归档：** 本需求的主要风险来自“鉴权结果复用”和“跨用户 URL 访问”，核心信任边界是用户请求、实训环境入口、authen center 和 Pod-ID 绑定信息。
...
flowchart TB
    ...
    subgraph T3[信任边界3: authen center]
        C[用户身份解析]
    end
    subgraph T4[信任边界4: Pod-ID 绑定信息]
        P[ownername]
    end
    U -->|UT/YG + Pod-ID| G
    G -->|查询身份| C
...
|-----------|-------------------------|---------------|----------------------------|
| **信息泄露** | 通过共享 URL 进入他人实训环境，读取或修改环境中的数据 | 高 | 每次访问都实时解析 `UT/YG` 对应 username，并与 Pod-ID 绑定 ownername 比对 |
| **篡改/伪造** | 伪造请求主体或复用旧缓存结果绕过鉴权 | 高 | 移除缓存复用逻辑，所有请求都回源 authen center 校验 |
| **权限提升** | B 用户复用 A 用户的短时缓存结果进入 A 的环境 | 高 | 删除鉴权缓存，禁止跨用户复用授权结果 |
| **拒绝服务** | 频繁访问导致 authen center 压力上升 | 中 | 保持最小化校验链路，必要时通过 authen center 侧限流缓解 |
...
本需求采用“UT/YG 对应 username + Pod-ID + ownername”三方实时校验，xihe-server 仅负责比对，移除缓存结果成为跨用户越权入口的可能性。

* **身份认证与授权**：从现有请求中提取 `UT/YG`，进入实训环境时必须实时解析出 username，并与 Pod-ID 绑定 ownername 进行比对。
* **缓存移除**：删除鉴权缓存数据结构、命中分支和写入逻辑，不再保存可跨请求复用的授权结果。
* **失败处理**：鉴权失败直接返回 401/403，不继续执行环境进入逻辑；不返回内部实现细节。
* **审计日志**：记录 `UT/YG`、Pod-ID、解析出的 username、ownername、授权结果、原因码；不记录完整 token、cookie 或环境敏感内容。
...
| **TASK2** | 改造鉴权链路为每次实时查询 authen center | chenqi |
...
**设计说明/归档：** 不涉及独立可靠性机制变更。该需求不引入新组件，也不改变服务可用性架构；实时鉴权会增加对 authen center 的调用频率，需要依赖 authen center 的基础稳定性。
...
**设计说明/归档：** 鉴权缓存移除后，每次访问都实时回源 authen center；因此该需求接受一定的鉴权时延增加，但必须保证不引入明显的访问失败或阻塞。
...
| TASK2 | 改为实时鉴权并回源 authen center | 拒绝越权访问的逻辑 | chenqi |
