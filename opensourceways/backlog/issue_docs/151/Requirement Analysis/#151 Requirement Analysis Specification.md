...
**场景说明:** 当前 MindSpore 实训沙箱会缓存已登录用户的鉴权结果，缓存有效期内不会再次向 authen center 校验 `UT/YG` 对应的 username。实训环境访问时，需要根据 `UT/YG` 解析出 username，再与 Pod-ID 绑定的 ownername 比对。若 A 用户已进入实训环境，B 用户可直接复用 A 用户环境 URL，在缓存窗口内无鉴权进入并操作 A 用户的实训环境，造成越权访问风险。本次需求要求移除该缓存复用逻辑，改为每次访问都实时鉴权。
...
**逻辑方案:** 直接移除 MindSpore 实训沙箱中“同一用户鉴权结果短时缓存后直接复用到环境访问”的逻辑，改为在环境访问链路中对当前请求进行实时校验。每次访问都应携带 `UT/YG`，xihe-server 实时查询 authen center 解析出 username，再与 Pod-ID 绑定的 ownername 比对，确保环境 URL 只能被所属用户使用。

```mermaid
flowchart LR
    A[用户访问实训环境URL] --> B[携带 UT/YG]
    B --> C[服务端鉴权]
    C --> D[实时查询 authen center]
...
| **TASK2** | 改造环境访问链路为实时鉴权，确保每次访问都走 authen center 校验并与 Pod-ID ownername 比对 | 实时鉴权逻辑代码 | 1.5 |
...
* [x] **跨组件影响**：变更会触发下游服务或关联系统的连锁反应（级联效应）。**原因：实时鉴权会更频繁访问 authen center 并读取 Pod-ID 绑定信息。**
