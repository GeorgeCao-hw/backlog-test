# #151 MindSpore实训沙箱鉴权机制修改 需求分析说明书
...
**场景说明:** 当前 MindSpore 实训沙箱对已登录用户的鉴权结果存在缓存，缓存有效期内不会再次向账号中心校验用户身份。若 A 用户已进入实训环境，B 用户可直接复用 A 用户环境 URL，在缓存窗口内无鉴权进入并操作 A 用户的实训环境，造成越权访问风险。
...
**逻辑方案:** 调整 MindSpore 实训沙箱的鉴权策略，去除“同一用户鉴权结果短时缓存后直接复用到环境访问”的行为，改为在环境访问链路中对当前请求主体进行实时校验，并确保环境 URL 只能被所属用户使用。必要时补充会话绑定或二次校验，避免不同用户共享同一鉴权结果。

```mermaid
flowchart LR
    A[用户访问实训环境URL] --> B[服务端鉴权]
    B --> C{是否为当前用户}
    C -- 是 --> D[允许进入环境]
    C -- 否 --> E[拒绝访问/重新鉴权]
```
...
|---------|---------------------------|----------------------|----------------|
| **TASK1** | 调整 MindSpore 实训沙箱鉴权逻辑，移除或修正跨用户复用的缓存校验方式 | 鉴权逻辑代码修订 | 1.5 |
| **TASK2** | 补充访问主体绑定与异常拦截，确保环境 URL 仅对所属用户有效 | 访问控制相关代码与配置 | 1 |
...
* [x] **权限调整**：修改权限模型、服务账号（SA）权限或鉴权逻辑。**原因：本需求的核心是修正实训环境的鉴权/授权链路，防止越权访问。**
...
* [x] A环节判定需要完成安全设计**原因：鉴权缓存导致越权访问，必须补充安全设计与边界校验。**
* [ ] 改变了现有系统的物理/逻辑拓扑
...
* [x] 上述环节判定需要执行安全设计或架构设计。**原因：鉴权链路修复后必须做串测验证。**
* [x] **跨组件影响**：变更会触发下游服务或关联系统的连锁反应（级联效应）。**原因：实训沙箱访问依赖账号中心和环境服务协同。**
...
| **必要性**   | 现有组件通过配置变更是否无法实现目标或没有不用开发的替代方案？ | 是，需要调整鉴权逻辑才能消除越权窗口 |
| **工作量**   | 预计总工作量？        | 3 人天 |
...
**原因描述:** 该需求属于明确的安全修复项，直接解决 MindSpore 实训沙箱的越权访问问题，能消除第三方通过共享 URL 无鉴权进入他人环境的风险，具有明确的安全价值与业务必要性。
