...
**逻辑方案:** 直接移除 MindSpore 实训沙箱中“同一用户鉴权结果短时缓存后直接复用到环境访问”的逻辑，改为在环境访问链路中对当前请求进行实时校验。每次访问都应携带 `UT/YG`，服务端实时查询账号中心解析出 username，再与 Pod-ID 绑定的 ownername 比对，确保环境 URL 只能被所属用户使用。
