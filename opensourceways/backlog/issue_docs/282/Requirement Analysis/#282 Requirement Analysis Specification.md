# #282 Bot支持识别Issue状态并自动标记label需求分析说明书
...
测试人员在 Gitcode 社区中使用 Issue 状态字段（TODO/ACCEPTED/WIP/VALIDATION/DONE/REJECTED）管理 Issue 生命周期，但现有 Bot 未能感知这些状态变化，导致 `resolved` 标签无法随状态自动打标。需要 Bot 监听 Issue 状态变更事件，对处于 `VALIDATION` 和 `DONE` 阶段的 Issue 自动添加 `resolved` label；同时统计看板需支持采集 Gitcode Issue 状态字段，补全数据维度。

**范围边界：**
- **在本次范围内**：Bot 识别 `VALIDATION`/`DONE` 状态自动打 `resolved` label；统计看板采集 Gitcode Issue 状态字段
- **不在本次范围内**：其他状态（TODO/ACCEPTED/WIP/REJECTED）的自动标记；移除 label 的逻辑
...
flowchart LR
    A["Gitcode Issue 状态变更"] --> B["Bot 接收 Webhook 事件"]
    B --> C{"状态 == VALIDATION 或 DONE?"}
    C -->|是| D["调用 API 打 resolved label"]
    C -->|否| E["忽略"]

    F["统计看板定时采集"] --> G["采集 Gitcode Issue 状态字段"]
    G --> H["存储至数据表"]
```
...
|---------|------------------------------|-------------------------|-----------------|
| **task1** | Bot 侧：监听 Gitcode Issue 状态变更 Webhook，识别 `VALIDATION`/`DONE` 状态并自动打 `resolved` label（含单测） | Bot 事件处理逻辑代码 | 2 |
| **task2** | 统计看板侧：新增 Gitcode Issue 状态字段数据采集逻辑，存储并支持查询 | 数据采集模块 + 数据表变更 | 2 |
...
* [x] **跨组件影响**：变更会触发下游服务或关联系统的连锁反应（级联效应）。原因：Bot 行为变更（自动打标）会影响下游依赖 `resolved` label 的流程（如 Issue 关闭、统计看板等）。
...
| 维度 | 评估问题 | 结论/说明 |
|------|---------|----------|
| **范围判定** | 该需求是否属于基础设施范围内？ | 是，Bot 自动化标记和数据采集均属于基础设施能力 |
| **规划一致性** | 该需求是否在年度技术规划中？ | 是，Gitcode 社区 Issue 管理流程优化的配套需求 |
| **优先级** | 该需求优先级评估（高/中/低）？ | 中 |
| **通用性** | 该需求是否解决 3 个以上业务方的共性痛点？ | 是，所有使用 Gitcode Issue 状态字段的测试团队均受益 |
| **必要性** | 现有组件通过配置变更是否无法实现目标或没有不用开发的替代方案？ | 是，需开发 Webhook 事件处理逻辑和数据采集模块 |
| **工作量** | 预计总工作量 | 4 人天 |
| **价值评估** | 实现后能减少多少手动操作或提升多少系统稳定性？ | 消除测试人员手动打 `resolved` label 的操作，看板数据维度更完整 |

> **状态定义：** **Accept (准入)** | **Reject (驳回)** | **Pending (待议)**

**建议结论**： Accept

**原因描述:** 该需求解决测试团队 Issue 状态字段与 Bot 标记脱节的问题，通过自动化打标消除手动操作，同时补全统计看板数据维度，实现成本低（4人天），价值明确。
