...
在 `2.0.1` 修订版文档发布后，系统应支持用户同时查看最新版、当前修订版以及所有历史版本文档，以便在不同版本之间进行对比与追溯。
...
- 页面默认展示最新版本文档的查看入口。
- 通过页面提供的“全部版本”入口，用户可查看该文档的所有历史版本列表。
...
- 分别获取 `2.0.0` 版本文档列表与 `2.0.1` 版本文档列表。
- 以 `type_name` 为合并键合并两份文档列表。
- 通过 `type_name` 获取某篇文档关联的全部版本信息。

### 4.2 任务清单

| 任务 ID     | 任务描述 (Task Description) | 预期产出 (Deliverables)              | 预期工作量（人天） |
|-----------|-------------------------|----------------------------------|-----------|
| **TASK1** | 获取文档列表并执行合并	           | `packages/website/stores/docs.ts` | 3         |
| **TASK2** | 展示某篇文档的全部版本                      | `packages/website/views/docs/components/DocsVersionsTable.vue`                                 | 3         |
...
| **工作量**   | 预计总工作量                          | 8 人天  | 
| **价值评估**  | 实现后能减少多少手动操作或提升多少系统稳定性？         | 无     |

> **状态定义：** **Accept (准入)** | **Reject (驳回)** |  **Pending (待议)**

**建议结论**： Accept

**原因描述:** 在 `2.0.1` 版本文档发布后，用户需能够查看修订版、完整版以及所有历史版本文档，以满足版本追溯与内容比对的使用场景。
