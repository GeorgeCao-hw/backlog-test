...
* [ ] A环节判定需要完成安全设计。_原因：A 环节勾选了边界变更和凭证处理，触发 need_security。_
* [ ] 改变了现有系统的物理/逻辑拓扑。_原因：蓝区新增 CI Runner 节点，改变了 CI 基础设施的节点拓扑。_
...
* [ ] 上述环节判定需要执行安全设计或架构设计。_原因：A/B 环节均触发，需要对 Runner 网络访问和凭证安全进行集成验证。_
