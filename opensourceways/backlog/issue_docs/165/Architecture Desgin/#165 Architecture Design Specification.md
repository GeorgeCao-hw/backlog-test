...
SELECT
    ...
    max(CASE WHEN metric_id = 'cpu_util' THEN metric_value END) AS cpu_usage,
    max(CASE WHEN metric_id IN ('mem_util', 'mem_usedPercent', 'mem_usage') THEN metric_value END) AS mem_usage,
    region,
