# #1 【openUBMC文档更新】需求分析说明书

## 1. 基础信息

* **需求链接**: https://github.com/opensourceways/backlog/issues/187
* **需求名称**: openUBMC文档更新
* **开发责任人**: liuyong2000139
...
OpenUBMC 文档中心的内容以 Markdown 格式存放在 Git 仓库中。当文档内容需要更新（如功能迭代、内容修正、版本发布）时，运营人员需要能够自主提交文档变更，而无需依赖研发人员代为操作。
...
- 运营人员知晓文档源文件的 Git 仓库地址及所在分支/目录结构。
- 运营人员能够通过 Git 平台（如 Gitcode、GitHub）的 Web 界面，对 Markdown 文件进行在线编辑或上传替换。
- 运营人员提交的变更以 Pull Request（或 Merge Request）形式进入审核流程。
- 文档更新有明确的审核责任人（如技术负责人或文档负责人），审核通过后合并发布。
- 站点在合并后手动触发构建部署，更新后的文档在前台可见。
...
- 仓库准备：确认文档站点对应的 Git 仓库及分支策略（如 main 分支为发布分支）。
- 权限配置：为运营人员在 Git 平台上开通仓库的写入权限（或 Fork 权限）。
- 流程规范：制定文档更新 SOP，包括：编辑文件 → 提交 PR → 填写变更说明 → 指定审核人 → 审核通过合并 → 手动部署。
...
|-----------|-------------------------------|---------------------|-----------|
| **TASK1** | 为运营人员开通 Git 平台权限，并进行基础操作培训  	 | -                   | 1         |
| **TASK2** | 运营人员提交PR更新                    | `**.md`             | 1         |
...
| **工作量**   | 预计总工作量                          | 2 人天                       | 
| **价值评估**  | 实现后能减少多少手动操作或提升多少系统稳定性？         | 使运营人员具备自主更新文档的能力，减少跨部门沟通成本 |

> **状态定义：** **Accept (准入)** | **Reject (驳回)** |  **Pending (待议)**

**建议结论**： Accept

**原因描述:** 通过规范化 Git 仓库的文档更新流程，使运营人员能够自主提交文档变更，无需依赖研发人员代为操作。本需求不涉及代码开发，仅需配置权限、制定操作指引。
